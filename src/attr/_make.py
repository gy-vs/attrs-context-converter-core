...
class Converter:
    ...
    def __getstate__(self):
        """
        Return a dict containing only converter and takes_self -- the rest
        gets computed when loading.
        """
        return {
    ...
    *last* value.
    ...
    if not converters:
        ...
        if isinstance(last, Converter):
            # The wrapped callable carries the return annotation; going
            # through __call__ works on all Python versions, unlike
            # inspect.signature on the Converter instance itself.
            last = last.__call__
